    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div className="bg-slate-900 rounded-2xl border border-white/[0.08] p-8">
        ...
        <form onSubmit={handleKYCSave} className="space-y-4">
          <label className="flex items-center gap-3 p-4 bg-white/[0.04] border border-white/[0.08] rounded-xl cursor-pointer hover:bg-white/[0.08] transition">
            ...
            <span className="font-bold text-slate-300">ID-kontroll genomförd (KYC)</span>
          </label>
          <label className="flex items-center gap-3 p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl cursor-pointer hover:bg-amber-500/20 transition">
            <input type="checkbox" checked={pepStatus} onChange={(e) => setPepStatus(e.target.checked)} className="w-5 h-5 text-amber-600 rounded" />
            <span className="font-bold text-amber-200">Politiskt Exponerad Person (PEP)</span>
          </label>
          <div>
            <label className="block text-sm font-bold text-slate-400 mb-2">Compliance-anteckningar</label>
            <textarea 
              value={kycNotes} onChange={(e) => setKycNotes(e.target.value)} rows={3}
              placeholder="Dokumentation gällande medlens ursprung etc..."
              className="w-full border border-white/10 p-3 rounded-xl focus:ring-2 focus:ring-indigo-500/50 outline-none bg-white/[0.05] text-white" 
            />
          </div>
      ...
      <div className="bg-slate-900 rounded-2xl border border-red-500/10 p-8 flex flex-col justify-center items-center text-center">
        <AlertOctagon className="w-12 h-12 text-red-500 mb-4" />
        <h3 className="text-xl font-bold text-slate-100 mb-2">GDPR: Rätten att bli glömd</h3>
        ...
        <button onClick={handleGDPRAnonymize} className="w-full bg-red-500/10 text-red-400 border border-red-500/20 font-bold py-3 rounded-xl hover:bg-red-600 hover:text-white transition flex justify-center items-center gap-2">
