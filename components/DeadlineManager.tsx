...
export default function DeadlineManager({ caseId }: { caseId?: string }) {
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      ...
      default:
        return 'bg-white/[0.04] border-white/[0.08] text-slate-300'
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 rounded-2xl border border-white/[0.08] p-6">
        ...
        {showForm && caseId && (
          <form onSubmit={handleCreate} className="mb-4 p-4 bg-white/[0.04] rounded-lg border border-white/[0.08]">
        ...
        <div className="bg-slate-900 rounded-2xl border border-white/[0.08] p-6">
          ...
          <div className="space-y-2">
            {completedDeadlines.map((deadline) => (
              <div key={deadline.id} className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg opacity-70">
                <div className="font-semibold text-emerald-300 line-through">{deadline.title}</div>
